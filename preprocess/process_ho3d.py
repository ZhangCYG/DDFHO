...
parser.add_argument('--output_root', default='processed_data/ho3d/', help="Path to dataset root")
...
shape_dir = 'database/models/'
pose_dataset = os.listdir(shape_dir)

for i in tqdm(range(0, len(pose_dataset))):
    model_path = pose_dataset[i]
    obj_mesh = trimesh.load(shape_dir + model_path + '/textured_simple.obj', force='mesh')
    print(model_path)
