...
class BaseData(Dataset):
    ...
    def get_ddf_files(self, cad_idx):
        # sdf_dir = osp.join(self.cfg.DB.DIR, self.dataset, 'ddf_obj')
        sdf_dir = osp.join(self.cfg.DB.DIR, 'mesh_ddf', 'ddf_obj', self.dataset)
        filename = osp.join(sdf_dir, cad_idx + '.npy')
        assert osp.exists(filename), 'Not exists %s' % filename
        return filename
