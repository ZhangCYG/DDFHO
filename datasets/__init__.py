...
def build_dataset_ddf(args, name, cls, split, is_train=True, **kwarg):
    print(name)
    if name.startswith('obman'):
        from .obman import Obman as dset
    elif name.startswith('ho3d'):
    ...
    from .ddf_img import DdfImg
    dset = DdfImg(args, dset, is_train, **kwarg)
...
def build_dataloader_ddf(cfg, split, is_train=True, shuffle=None, name=None, cls=None, bs=None) -> DataLoader:
    if shuffle is None:
        shuffle = is_train
    print(cfg.MODEL.BATCH_SIZE)
    if bs is None:
    ...
    dataset, total_len = [], 0
    for each_name in name.split('+'):
        dset = build_dataset_ddf(cfg, each_name, cls, split, is_train, base_idx=total_len)
        total_len += len(dset)
        dataset.append(dset)
        # dataset = [build_dataset(cfg, each_name, cls, split, is_train) for each_name in name.split('+')]
    if '+' in name:
